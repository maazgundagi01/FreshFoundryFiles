...
$stmt = $db->prepare("SELECT * FROM $cart_user");
    ...
    echo '<div style="width:100%"><h1 style="margin-bottom:2rem">My Cart</h1><table style="border-radius:7px; "><form action="removeitem.php" method="POST"><th></th><th><h2>Product</h2></th><th></th><th><h2>Action</h2></th>';
    foreach ($data as $row) {
        echo '<tr >'.
        '<td><img style="height:40px;" src="../uploads/'. $row['good_image'].'"></td>
        <td><h5>'.$row['good_name']."</h5></td>
        <td><h5>Price: $".$row['good_price'].'</h5></td>
        <td><input value="'. $row['sr_no'].'" name="sr_no" type="hidden"><input style="padding: 10px;border-radius:10px;border: none; box-shadow: 1px 1px 3px rgba(0,0,0,0.3)" type="submit" name="submit" value="Remove"/></td></tr>';
    }
    echo '';
    ...
    echo '</form></table><table  ><tr style="box-shadow:none !important;"><th style="box-shadow:none !important;"><form action="pewpew.php" method="POST"> <label>Deliver to:</label></th></tr><tr><td><textarea style=\'border-radius:10px;border:1 px solid light-grey;padding: 1em;\' cols=\'30\' placeholder=\'Please enter your address here!\' name="user_address"></textarea><div><input name="submit" style="margin-bottom: 1rem; padding: 10px;border-radius:10px;border:none;box-shadow: 1px 1px 3px rgba(0,0,0,0.3)" type="submit" value="Place order"></div></td></tr></form></table></div>' ;
    }
